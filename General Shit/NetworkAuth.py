...
WS_PORT = config.AppConfiguration.WebsocketServer['Port']  # WebSocket server port
...
class NetworkAuthBroadcaster:
    ...
    def _process_broadcast(self, data, addr, application_session, on_auth_callback):
        ...
        try:
            ...
            remote_ws_host = message.get('ws_host')
            remote_ws_port = message.get('ws_port')
            ...
            if hasattr(application_session, 'UserKey'):
                expected_hash = hashlib.sha256(
                    f"{application_session.UserKey}{salt}{timestamp}".encode()
                ).hexdigest()
                
                if auth_hash == expected_hash:
                    self.system_log(f"Verified authentication from {addr[0]} ({hostname})")
                    self.system_log(f"Remote WebSocket server: {remote_ws_host}:{remote_ws_port}")
